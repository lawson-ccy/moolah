  ...
  IStableSwap
{
  ...
  /// @dev return oracle price for a given coin in 1e18 precision
  function getOraclePrice(address _coin) public view returns (uint256 oraclePrice) {
    require(oracle != address(0), "Oracle not set");
    return IOracle(oracle).peek(_coin) * 1e10; // Convert to 1e18 precision
  }
  ...
  function checkPriceDiff() public view {
    ...
    uint256 priceDiff0 = (poolPrice0 > oraclePrice0) ? poolPrice0 - oraclePrice0 : oraclePrice0 - poolPrice0;
    uint256 priceDiff1 = (poolPrice1 > oraclePrice1) ? poolPrice1 - oraclePrice1 : oraclePrice1 - poolPrice1;

    console.log("priceDiff0:", priceDiff0);
    console.log("priceDiff1:", priceDiff1);
    console.log("oraclePrice0:", oraclePrice0);
    console.log("oraclePrice1:", oraclePrice1);
    console.log("poolPrice0:", poolPrice0);
    console.log("poolPrice1:", poolPrice1);

    // Check if price differences exceed thresholds
    require(priceDiff0 <= (oraclePrice0 * price0DiffThreshold) / 1e18, "Price difference for token0 exceeds threshold");

    require(priceDiff1 <= (oraclePrice1 * price1DiffThreshold) / 1e18, "Price difference for token1 exceeds threshold");
  }
